...
def determine_solution(file_handle):
    ...
    for _ in range(num_cases):
        ...
        cow_preferences = list(map(int, file_handle.readline().rstrip().split(' ')))
        majorities = get_hay_solutions(cow_preferences)
        solutions.append(print_cow_preferences(majorities))
    return "\n".join(solutions) + "\n"

def get_hay_solutions(cow_preferences):
    solutions = set()
    if cow_preferences[0] == cow_preferences[1]:
        solutions.add(cow_preferences[0])
    for i in range(2, len(cow_preferences)):
        if cow_preferences[i - 2] == cow_preferences[i]:
            solutions.add(cow_preferences[i])
        if cow_preferences[i - 1] == cow_preferences[i]:
            solutions.add(cow_preferences[i])
    return sorted(list(solutions))

def print_cow_preferences(solution):
    if len(solution) == 0:
        return "-1"
